...
predfolder <- "/home/tnaum/data/BLMsoils/CaCO3_2D_SSURGO_NASIS_SCD"
...
pts.ext$prop <- pts.ext$caco3_r ## UPDATE EVERY TIME
...
## SCD prep for RF
scd.pts.ext$prop <- scd.pts.ext$caco3 ## UPDATE everytime!
scd.pts.ext$tid <- "scd"
...
pts.extcc$prop_t <- log(pts.extcc$prop+1) ## TRANSFORM IF NEEDED
attach(pts.extcc)
...
rasterOptions(maxmemory = 1e+09,chunksize = 1e+08)
setwd("/home/tnaum/data/BLMsoils/CaCO3_2D_SSURGO_NASIS_SCD")
...
watermask_fn <- function(g){
  ...
  setwd("/home/tnaum/data/BLMsoils/CaCO3_2D_SSURGO_NASIS_SCD/masked")
