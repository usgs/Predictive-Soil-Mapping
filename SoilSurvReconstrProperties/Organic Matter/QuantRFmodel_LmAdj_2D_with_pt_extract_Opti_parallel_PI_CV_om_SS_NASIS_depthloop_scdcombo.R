...
predfolder <- "/home/tnaum/data/BLMsoils/OM_2D_SSURGO_NASIS_SCD"
...
pts.ext$prop <- pts.ext$om_r ## UPDATE EVERY TIME
prop <- "om_r" ## Dependent variable
pts.ext$tid <- "nasis"

## Remove lab pedons from nasis data to allow for evaluation at end
scd.pts <- read.delim("//media/tnaum/D/GIS_Archive/NRCS_pedons/NCSS_Soil_Characterization_Database20170718/NCSS_Soil_Characterization_Database_05_17_2017_FGDB/NCSS17_SOC_Extractions_ttab.txt")
...
# scd.pts.ext.dcm <- subset(scd.pts.ext, as.numeric(scd.pts.ext$hzn_top) <= d & as.numeric(scd.pts.ext$hzn_bot) > d) ## SCD not used in training for OM
...
pts.extcc <- pts.extc # Not SCD locations were used
...
pts.extcc$prop_t <- log(pts.extcc$prop+0.1) ## TRANSFORM IF NEEDED
attach(pts.extcc)
...
pred_bt <- clusterR(predlm, calc, args=list(fun=bt.fn),progress='text')
...
pts.extcvm$prop_t <- log(pts.extcvm$prop+0.1) ## UPDATE: tranform if needed else just create new version of prop
...
pts.extpcv$pcvpred_bt <- exp(pts.extpcv$pcvpred)-0.1
...
relPI.depth <- function(d, pts.ext, pred.pts.extdf,Qsoiclass){
  ...
  ## Untransform
  DF_pred_l_bt <- exp(DF_pred_l) - 0.1 # Update for all runs, just don't use back transform fn if it wasn't used
  DF_pred_h_bt <- exp(DF_pred_h) - 0.1 # Update for all runs, just don't use back transform fn if it wasn't used
...
rasterOptions(maxmemory = 1e+09,chunksize = 1e+08)
setwd("/home/tnaum/data/BLMsoils/OM_2D_SSURGO_NASIS_SCD")
...
watermask_fn <- function(g){
  ...
  setwd("/home/tnaum/data/BLMsoils/OM_2D_SSURGO_NASIS_SCD/masked")
