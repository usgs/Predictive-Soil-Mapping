...
predfolder <- "/home/tnaum/data/BLMsoils/EC_2D_NASIS_SSURGO_SCD"
...
pts.ext$prop <- pts.ext$ec_r ## UPDATE EVERY TIME
...
## SCD prep for RF
scd.pts.ext$prop <- scd.pts.ext$ec_satp ## UPDATE everytime!
scd.pts.ext$tid <- "scd"
...
lines(x1,y1, col = 'red')#1:1 line
plot(logytrain~pts.extcc$trainpredsadj) #Fit plot
lines(x1,y1, col = 'red')#1:1 line
varImpPlot(soiclass)
setwd(predfolder)
...
setwd(covfolder)
rasterOptions(maxmemory = 3e+08,chunksize = 2.5e+07)# maxmemory = 1e+09,chunksize = 1e+08 for soilmonster
...
pts.extpcv$pcvpred_bt <- ifelse(pts.extpcv$pcvpred_bt<0,0,pts.extpcv$pcvpred_bt)
...
rasterOptions(maxmemory = 1e+09,chunksize = 1e+08)
setwd("/home/tnaum/data/BLMsoils/EC_2D_NASIS_SSURGO_SCD")
...
watermask_fn <- function(g){
  ...
  setwd("/home/tnaum/data/BLMsoils/EC_2D_NASIS_SSURGO_SCD/masked")
