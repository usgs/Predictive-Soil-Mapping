...
predfolder <- "/home/tnaum/data/BLMsoils/Clay_SSURGO_NASIS_SCD"
...
pts.ext$prop <- pts.ext$claytotal_r ## UPDATE EVERY TIME
...
## SCD prep for RF
scd.pts.ext$prop <- scd.pts.ext$clay_tot_psa ## UPDATE everytime!
scd.pts.ext$tid <- "scd"
...
pts.extcc$prop_t <- sqrt(pts.extcc$prop) ## TRANSFORM IF NEEDED
attach(pts.extcc)
...
pts.extcvm$prop_t <- sqrt(pts.extcvm$prop) ## UPDATE: tranform if needed else just create new version of prop
...
pts.extpcv$pcvpred_bt <- (pts.extpcv$pcvpred)^2 ## UPDATE EVERY TIME
...
relPI.depth <- function(d, pts.ext, pred.pts.extdf,Qsoiclass){
  ...
  ## Untransform
  DF_pred_l_bt <- (DF_pred_l)^2 # Update for all runs, just don't use back transform fn if it wasn't used
  DF_pred_h_bt <- (DF_pred_h)^2 # Update for all runs, just don't use back transform fn if it wasn't used
...
rasterOptions(maxmemory = 1e+09,chunksize = 1e+08)
setwd("/home/tnaum/data/BLMsoils/Clay_SSURGO_NASIS_SCD")
...
watermask_fn <- function(g){
  ...
  setwd("/home/tnaum/data/BLMsoils/Clay_SSURGO_NASIS_SCD/masked")
