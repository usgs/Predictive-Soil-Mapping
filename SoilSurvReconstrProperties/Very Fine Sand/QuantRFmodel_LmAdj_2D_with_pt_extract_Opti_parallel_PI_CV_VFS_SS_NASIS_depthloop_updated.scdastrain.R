...
predfolder <- "/home/tnaum/data/BLMsoils/VFS_2D_NASIS_SSURGO_SCD"
...
pts.ext$prop <- pts.ext$sandvf_r ## UPDATE EVERY TIME
...
## SCD prep for RF
scd.pts.ext$prop <- scd.pts.ext$sand_vf_psa ## UPDATE everytime!
scd.pts.ext$tid <- "scd"
...
#lines(x1,y1, col = 'red')#1:1 line
## CV plots
scaleFUN <- function(x) round(x,0) #for rounding count labels in CV plots
...
#   xlab("Measured") + ylab("CV Prediction") + scale_fill_gradientn(name = "log(Count)", trans = "log", colours = rev(viri),labels=scaleFUN) +
#   ggtitle(paste("Cross val", prop, d, "cm",sep=" "))
# gplt.dcm.2D.CV
# gplt.dcm.2D.RPI <- ggplot(data=pts.extpcv, aes(RPI, abs.resid)) +
#   stat_binhex(bins = 30) + #geom_abline(intercept = 0, slope = 1,lwd=1)  + #xlim(0,100) + ylim(0,100) +
#   theme(axis.text=element_text(size=8), legend.text=element_text(size=10), axis.title=element_text(size=10),plot.title = element_text(size=10,hjust=0.5)) +
#   xlab("Relative 95% Prediction Interval") + ylab("Absolute CV Residuals") + scale_fill_gradientn(name = "log(Count)", trans = "log", colours = rev(viri),labels=scaleFUN) +
#   ggtitle(paste("RPI", prop, d, "cm",sep=" "))
# gplt.dcm.2D.RPI
...
rasterOptions(maxmemory = 1e+09,chunksize = 1e+08)
setwd("/home/tnaum/data/BLMsoils/VFS_2D_NASIS_SSURGO_SCD")
...
watermask_fn <- function(g){
  ...
  setwd("/home/tnaum/data/BLMsoils/VFS_2D_NASIS_SSURGO_SCD/masked")
